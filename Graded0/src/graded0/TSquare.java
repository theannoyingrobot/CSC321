...
public class TSquare extends JPanel {
    ...
    private List<Rectangle2D> moreRects(Rectangle2D base) {
        ...
        //upper right square
        Rectangle2D upR = new Rectangle2D.Double(ulx + .75 * l, uly - .25 * l, l / 2, l / 2);
        rects.add(upR);
        rects.addAll(moreRects(upR));
        //upper left square
        Rectangle2D upL = new Rectangle2D.Double(ulx - .25 * l, uly - .25 * l, l / 2, l / 2);
        rects.add(upL);
        rects.addAll(moreRects(upL));
        //lower right square
        Rectangle2D lowR = new Rectangle2D.Double(ulx + .75 * l, uly + .75 * l, l / 2, l / 2);
        rects.add(lowR);
        rects.addAll(moreRects(lowR));
        //lower left square
        Rectangle2D lowL = new Rectangle2D.Double(ulx - .25 * l, uly + .75 * l, l / 2, l / 2);
        rects.add(lowL);
